...
function createAlbum() {
	...
	var chooseAlbum = function(){ // 选择专辑事件
	...
	return (
	tipsbox("<div id='tipsbox'><div class='tipsBox-title'><h4>创建专辑</h4></div><div class='tipsBox-main'><div class='add-album'><div class='create-album'><div class='album-list-wrap'><div class='tips-album-list'>"+ html() +" </div><div class='create-album-text'><input type='text' id='album_name' name='album_name' placeholder='快速创建专辑' /><a href='javascript:;' id='create-album-btn'>创建</a></div></div></div></div></div></div>",
            function() {
            	chooseAlbum() // 选择专辑事件
            	c_Album() // 创建专辑事件
           	})
...
Vue.component('useralbum',{
	...
	props:['userAlist'],
	template : " <ul><li >{{defaultChoose}}</li><template v-for='item in userAlist'><li :data-aid=item.aid >{{item.album_name}}</li></template></ul>",
})
...
new Vue({
	...
	mounted:function () {
		var that = this
		//tipsbox init
		if($user.userid){ // 登录后获取标签和个人专辑列表
			var that = this,tagsObj=[]
	...
	methods:{
		...
		checkSubBtn:function(e){//submit check提交按钮监听
			e.preventDefault()

			if(!$("input[name='photoimg']").attr('value') && !$("input[name='titlepic']").attr('value')) {
				MsgBox('请上传图片'); return;
		...
		unCheckTag:function(e,index){ // 数组中移除
	...
	$.upload({
        ...
        success : function(data){
	        var data = JSON.parse(data.responseText);
	          var img = "<div class='pic-item'><img src="+ data.url +"><i class='del-pic-item'></i></div>"
