// retu-msg-page
define(['jquery','tipsbox','util'], function($, tipsbox, util) { // 引入jquery

	var type = typeof($sys) == 'undefined' ? '': $sys;

	if(type && type == 'xt' || type == 'gz') {
		$('.msg_list_right a,.gz_msg_c a').click(function(){
			var mid = $(this).data('mid');
			if(mid && confirm('确认删除吗?')) {
                $.post("/majax/delmsg/", {mid: mid}, function (result) {
                    if (result.status == 1) {
                        $("#msg" + result.mid).slideUp("slow");
                        return true;
                    }
                }, "json");
	        }else{
